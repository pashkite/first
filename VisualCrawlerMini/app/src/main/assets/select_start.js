(function(){
  ...
  function pick(ev){
    ...
    if(!el){window.VisualCrawler.onError('본문 시작점을 찾지 못했어.');return false;}
    let box=pickBlock(el);
    document.querySelectorAll('.__vc_range_start__').forEach(x=>x.classList.remove('__vc_range_start__'));
    box.classList.add('__vc_range_start__');
    const pv=clean(box.innerText||box.textContent).slice(0,500);
    badge('본문 시작 선택됨');
    window.VisualCrawler.onRangeStartSelected(cssPath(box), pv);
  ...
  window.VisualCrawler.onInfo('본문 시작 선택 모드: 실제 본문이 시작되는 첫 문장 근처를 터치해.');
})();